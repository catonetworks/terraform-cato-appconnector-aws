...
# Only run this data block if var.app_conn_location is null
# (Terraform does not support dynamic data blocks, so we use count)
data "cato_siteLocation" "app_conn_location" {
...
locals {
  ## Check for all app_conn_location inputs to be null
  all_location_fields_null = (
    var.app_conn_location.city_name == null &&
    var.app_conn_location.country_code == null &&
    var.app_conn_location.state_code == null &&
    var.app_conn_location.timezone == null
  ) ? true : false

  ## If all app_conn_location fields are null, use the data source to fetch the 
  ## app_conn_location from azure provuder location, else use var.app_conn_location
  cur_app_conn_location = local.all_location_fields_null ? {
    country_code = data.cato_siteLocation.app_conn_location[0].locations[0].country_code
    timezone     = data.cato_siteLocation.app_conn_location[0].locations[0].timezone[0]
    state_code   = data.cato_siteLocation.app_conn_location[0].locations[0].state_code
    city_name    = data.cato_siteLocation.app_conn_location[0].locations[0].city
  } : var.app_conn_location
...
output "app_conn_location" {
  value = data.cato_siteLocation.app_conn_location
}
